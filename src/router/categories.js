...
export const getCategories = async (c) => {
    try {
        ...
        const categories = await client.execute(`SELECT * FROM ${CATEGORY}  ORDER BY CreatedAt DESC LIMIT ? OFFSET ?`, "all", [
            pagesAndPageNumber.pages,
            (pagesAndPageNumber.pageNumbers - 1) * pagesAndPageNumber.pages,
        ]);
        //   获取分类总数
        const total = await client.execute(`SELECT COUNT(*) AS total FROM ${CATEGORY}`, "first");
        await client.close();
        return c.json({
            success: 1,
...
export const createCategory = async (c) => {
    ...
    try {
        ...
        const isExist = await transaction.execute(`SELECT * FROM ${CATEGORY} WHERE Title = ?`, "first", [Title]);
        ...
        // 插入分类
        const result = await client.execute(`INSERT INTO ${CATEGORY} (Title, ImgUrl, CreatedAt, UpdatedAt) VALUES (?,?,?,?)`, "write", [Title, ImgUrl, timeNow, timeNow]);
...
export const deleteCategory = async (c) => {
    try {
        ...
        const client = await sqlDb();
        const result = await client.execute(`DELETE FROM ${CATEGORY} WHERE CategoryId = ?`, "write", [CategoryId]);
        await client.close();
...
export const updateCategory = async (c) => {
    ...
    try {
        ...
        const client = await sqlDb();
        const transaction = await client.transactionFun("write");
        const category = await transaction.execute(`SELECT * FROM ${CATEGORY} WHERE CategoryId = ?`, "first", [CategoryId]);
        ...
        const timeNow = Date.now();
        // 判断分类名字是否已存在
        const isExistCategory = await transaction.execute(`SELECT * FROM ${CATEGORY} WHERE Title = ? and CategoryId != ?`, "first", [Title, CategoryId]);
        if (isExistCategory && isExistCategory.list) {
            await client.close();
            return errorStatusMessage(c, 409, "Category name");
        }
        // 更新分类
        const update = await client.execute(`UPDATE ${CATEGORY} SET ${updateedData.fields},UpdatedAt=?  WHERE CategoryId = ?`, "write", [...Object.values(updateedData.values), timeNow, CategoryId]);
